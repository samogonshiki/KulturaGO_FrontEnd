...
const Map: React.FC<Props> = ({ apiKey }) => {
  ...
  const [statistics] = useState<Statistic>({ cities: 4, places: 12, areaKm: 5.3 });
  const [lastVisited] = useState<VisitedPlace[]>([
    { id: '1', name: 'Dontea Tea Bar', address: 'Jl. Gajah Mada No.115…', type: 'cafe', icon: 'local_cafe' }
  ]);
  const [routePoints, setRoutePoints] = useState<{ from: SearchPoint; to: SearchPoint }>({
    from: { address: '' },
    to: { address: '' }
  });
  ...
  const [markers, setMarkers] = useState<{ A: { lat: number; lng: number } | null; B: { lat: number; lng: number } | null }>({ A: null, B: null });
